  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response.data && response.data.user_id) {
        // User info olish
        const userInfo = await axios.get(`${baseUrl}/user/${response.data.user_id}/`, {
          headers: {
      ...
      toast.error("Login yoki parol xato.", { id: toastId });
    } catch (error) {
      setLoading(false);
      setErrorMessage("Tizimda xatolik. Iltimos qaytadan urunib ko'ring.");
      toast.error(
        "Tizimga kirishda xatolik.Iltimos qaytadan urunib ko'ring.",
        { id: toastId }
      );
      console.log(error);
    }
    ...
    >
      <div className="sm:w-[578px] w-full sm:px-[118px] px-[20px] py-[40px] flex flex-col gap-4 sm:shadow-shadow6">
        ...
        <p className="text-black clamp3 font-[400] text-center">
          Tizimga kirish uchun kirish ma'lumotlarni to'ldiring
        </p>
        <motion.h1
        ...
        <div className="flex flex-col gap-[24px] text-thin">
          ...
          >
            Sizda account yo'qmi? <strong>Ro'yxatdan o'tish</strong>
          </button>
        </div>
      </div>
    </form>
